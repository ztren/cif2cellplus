...
for f in files:
    if f != 'Please_Put_Cifs_Here':
        if (filetype != 'n'):
            ...
            filedir = f[:-5]
        filename = '../'+filedir+'/System/'+filedir
        filedir = '../'+filedir+"/System"
        if not os.path.isdir(filedir):
            os.makedirs(filedir) #otherwise FileExistsError
        ...
        if MoveCell:
            ...
            shutil.copyfile('default/main_job_movexyz.sh',filedir+'/main_job_movexyz.sh')
        if (filetype != 'n'):
        ...
        subprocess.run(["sed -i ''s/#DONT_MODIFY_THIS/"+filedir[3:-7]+"/'' "+filedir+"/main_job*.sh"],shell=True)
        runall.write('cd '+filedir[3:]+'\nsbatch main_job.sh\ncd -\n')
